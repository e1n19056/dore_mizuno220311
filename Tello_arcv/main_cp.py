...
def main():
	...
	cv2.createTrackbar("H_max", "OpenCV Window", 31, 179, nothing)		# Hueの最大値は179
	cv2.createTrackbar("S_min", "OpenCV Window", 26, 255, nothing)
	cv2.createTrackbar("S_max", "OpenCV Window", 167, 255, nothing)
	cv2.createTrackbar("V_min", "OpenCV Window", 244, 255, nothing)
	...
	try:
		while True:
          ...
          # P制御の式(Kpゲインはとりあえず1.0)
					dx = 0.7 * (240 - mx)		# 画面中心との差分
					    #↑　ここのdxの数値は1.0最大。左右にブレるなら少し減らす
					# 旋回方向の不感帯を設定
...
#			    			    number=number+1
					...
					if idno == pre_idno:	# 今回認識したidnoが前回のpre_idnoと同じ時には処理
						...
						if count > 50:		# 50回同じマーカーが続いたら，コマンドを確定する
							...
							if idno == 0:
								...
								drone.move_forward(1.25)		# 前進
                                                                time.sleep(3)
							elif idno == 7:
								drone.move_backward(1.25)	# 後進
                                                                time.sleep(3)
							elif idno == 8:
								drone.move_left(1.35)		# 左移動
                                                                time.sleep(3)
							elif idno == 9:
								drone.move_right(1.35)		# 右移動
								time.sleep(3)
			...
			cv2.imshow('OpenAR Window', bgr_image)
